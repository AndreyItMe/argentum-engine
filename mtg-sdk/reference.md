...
- `AdditionalETBTriggers(creatureFilter, enteringMustBeYouControl = true)` — when a permanent matching the filter enters the battlefield, triggered abilities of your permanents that fired from that event trigger an additional time. Defaults to requiring the entering permanent to be under your control (Naban, Dean of Iteration; Traveling Chocobo). Set `enteringMustBeYouControl = false` for Starfield Vocalist–style cards where any controller's entering permanent counts.
